...
if(isset($_GET['num']) && isset($_GET['code_ets'])) {
    ...
    if(!empty($bordereau['DATE_DEMANDE'])) {
        ...
        foreach ($factures as $facture) {
			...
			$trouver_facture1 = $FACTURES->trouver($facture['NUM_FACTURE']);
			...
			$total_montant = $total_montant + $facture['MONTANT'];
            $total_part_cmu = $total_part_cmu + (round($facture['PART_CMU'])+$part_ac);
            ...
            $pdf->MultiCell(20, 0, number_format($part_assure,'0','',' '), 1, 'R', 0, 0, '', '', true);
            $pdf->MultiCell(20, 0, number_format(round($facture['PART_CMU'])+ $part_ac,'0','',' '), 1, 'R', 0, 1, '', '', true);
            $ligne++;
        }
        $pdf->SetFillColor(179, 179, 179);
        $pdf->SetFont('helvetica', 'B', 6.5);
