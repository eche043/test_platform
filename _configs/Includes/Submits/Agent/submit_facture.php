                ...
                if(!empty($facture['FEUILLE'])) {
                    ...
                    if(in_array($facture['STATUT'],$array_statut)) {
                        ...
                        if(!empty($num_fs_initiale)) {
                            ...
                            if(!$pathologies){
                                if($facture_initiale['AFFECTION1']){
                                    $pathologies[]=$facture_initiale['AFFECTION1'];
                                }
                                if($facture_initiale['AFFECTION2']){
                                    $pathologies[]=$facture_initiale['AFFECTION2'];
                                }
                            }
                        }
                        else{
                        ...
                        if($statut_mirka==true){
                            ...
                            if($type_facture=='EXP'){
                                ...
                                if(empty($type_ets_autre)){
                                    $type_ets_autre = 'BIOLOGIE';
                                }
                            }
                            if($pathologies){
                                $body = json_encode([
                                                ...
                                                if($supression_acte['status'] == true) {
                                                    ...
                                                    if($ligne == $nb_actes) {
                                                        ...
                                                        if($maj_facture['status'] == true) {
                                                            //if($statut_fse == 'F'){$statut_mirka = true;}
                                                            $json = array(
                                                                'status' => true,
                                    ...
                                    elseif ($type_facture == 'HOS' || $type_facture == 'EXP'){
                                                ...
                                                if($nb_actes != 0) {
                                                    ...
                                                    if($supression_acte['status'] == true) {
                                                        ...
                                                        if($ligne == $nb_actes) {
                                                            ...
                                                            $maj_facture = $FACTURES->maj_facture($type_facture,$date_soins,$num_facture,$facture_initiale['FEUILLE'], $num_ep_cnam, $num_ep_ac, $num_ac,$type_ets,$type_ets_autre,$info_complementaire,$info_compl_autre,NULL,NULL,$code_programme,$code_affection[0],$code_affection[1],$date_sortie,$type_sortie,$code_ps,$code_specialite_ps,'F',$utilisateur['ID_UTILISATEUR']);
                                                            if($maj_facture['status'] == true) {
                                                                //$statut_mirka = true;
                                        ...
                                        if(!empty($num_fs_initiale)) {
                                            ...
                                            if(!in_array($facture_initiale['STATUT'],$array_statut_fs_initiale)) {
                                                if($type_facture == 'MED') {
                                                    $type_acte = 'm';
                                                    $ps = $FACTURES->verifier_facture_ps($trouver_ets['CODE_PS'],NULL,$facture['ETABLISSEMENT'],strtoupper(date('Y-m-d',strtotime($facture['DATE_SOINS']))));
                                                }else {
                                                    $type_acte = 'a';
                                                ...
                                                if(isset($ps['code_ps'])){
                                                            ...
                                                            if($ligne == $nb_actes) {
                                                                ...
                                                                if($maj_facture['status'] == true) {
                                                                    ...
                                                                    if(empty($facture_initiale['PS'])) {
                                                                        $maj_ps = $FACTURES->maj_ps_facture($code_ps,$code_specialite_ps,$facture_initiale['FEUILLE']);
                                                                    }
                                                                    $json = array(
                                                                        'status' => true,
                                                                        'message' => $maj_facture['message']
                                                    ...
                                                    $json = array(
                                                        'status' => false,
                                                        'message' => "LE CODE PROFESSIONNEL DE SANTE NE CORRESPOND PAS A L'ETABLISSEMENT DE SANTE.".$trouver_ets['CODE_PS'].' -- '.$facture['ETABLISSEMENT']
                                                    );
                                                }
                                            }else {
                                                $json = array(
