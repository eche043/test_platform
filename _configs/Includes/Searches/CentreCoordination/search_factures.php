...
if(strlen($code_ets) == 9 && (!empty($type_facture) || !empty($date_soins) || strlen($num_facture) >= 2 || strlen($num_secu) >= 3 || strlen($nom) >= 3)) {
    include "../../../../_configs/Classes/FACTURES.php";
    $FACTURES = new FACTURES();
    $factures = $FACTURES->moteur_recherche_factures($code_ets,$type_facture,$date_soins,$num_facture,$num_secu,$nom);
        ...
        <table class="table table-sm table-bordered table-hover">
            ...
            foreach ($factures as $facture) {
                ...
                <tr>
                    ...
                    <td align="right"><a href="<?= URL.'centre-coordination/facture.php?type='.$facture['CODE_TYPE_FACTURE'].'&num='.$facture['FEUILLE'].'&code-ets='.$facture['ETABLISSEMENT'];?>"><b><?= $facture['FEUILLE'];?></b></a></td>
                    ...
                    <td>
                        <?php
                        if(empty($facture['STATUT']) || $facture['STATUT'] == 'C' || $facture['STATUT'] == 'N' || $facture['STATUT'] == 'F') {
                            if(empty($facture['NUM_BORDEREAU'])){$tip_bord = 'AUCUN BORDEREAU';}else{$tip_bord = 'Bordereau N°'.$facture['NUM_BORDEREAU'];}
                            echo '<a href="'.URL.'centre-coordination/facture-edition.php?type='.$facture['CODE_TYPE_FACTURE'].'&num='.$facture['FEUILLE'].'&code-ets='.$facture['ETABLISSEMENT'].'" class="badge badge-success tp_infos" data-toggle="tooltip" data-placement="left" title="'.$tip_bord.'" ><i class="fa fa-edit"></i></a>';
                        }else {
                            if(empty($facture['NUM_BORDEREAU'])){$tip_bord = 'AUCUN BORDEREAU';}else{$tip_bord = 'Bordereau N°'.$facture['NUM_BORDEREAU'];}
