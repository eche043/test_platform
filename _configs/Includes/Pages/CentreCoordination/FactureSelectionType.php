                            ...
                            if(!empty($facture['FEUILLE']))
                            {
                                if($ets['CODE_ETS'] == $facture['ETABLISSEMENT'])
                                {
                                    if(empty($facture['TYPE_FEUILLE'])) {
                                            ...
                                            <div class="row justify-content-md-center" style="margin-top: 100px">
                                                ...
                                                foreach ($types as $type) {
                                                    if($type['CODE'] != 'MED')
                                                    {
                                                        ?>
                                                        <div class="col-sm-3">
                                                            <a href="<?php if($type['CODE'] == 'HOS'){echo '#';}else {echo URL.'centre-coordination/facture-edition.php?type='.$type['CODE'].'&num='.$facture['FEUILLE'].'&code-ets='.$ets['CODE_ETS'];} ?>" <?php  if($type['CODE'] == 'HOS'){echo 'data-toggle="modal" data-target="#HOSModal"';} ?> style="height: 50px; line-height: 40px" class="btn btn-block btn-sm <?php if($type['CODE'] == 'HOS'){echo 'btn-danger';}else {echo 'btn-primary';} ?> box_profils"><?= str_replace(' - CONSULTATIONS','',$type['LIBELLE']);?></a>
                                                        </div>
                                                    <?php
                                                    if($type['CODE'] == 'HOS')
                                                    {
                                                    ?>
                                                        ...
                                                        <script type="text/javascript" src="<?= JS.'page_centre_coordination_facture.js'?>"></script>
                                                        <?php
                                                    }
                                                    }
                                                }
                                        ...
                                        echo '<script>window.location.href="'.URL.'centre-coordination/facture-edition.php?type='.$facture['TYPE_FEUILLE'].'&num='.$facture['FEUILLE'].'&code-ets='.$ets['CODE_ETS'].'"</script>';
                                    }
                                }
                                else
                                {
                                    echo '<script>window.location.href="'.URL.'centre-coordination/factures.php?code-ets='.$ets['CODE_ETS'].'"</script>';
                                }
